...
- **WO-075-IJ** — **implement the regime structural-tail layer → ship 0.9.48** (anchor [D-014-IJ] design + [D-018-IJ] timing): the injected `opts.regime` table + nil-guarded health-anchored estimator seams (freeze/immune/reset off `h`, **cap confidence so the bar goes quiet**) that a rhythm profile provably can't do. Three human-merged PRs (freeze+hideBar+lab · confCap+suppressFlush · resetOnRise), each gated by sim byte-identity + corpus regression. Onyxia's air phase is the motivating case. Full design: `docs/reference/estimator-regime-design.md` (see `docs/workorders/WO-075-IJ.md`). Status: Accepted.
