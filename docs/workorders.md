...
- **WO-069-IJ** — **estimator regime-aware overhaul** (fable + ultracode): stop stretching the solo-tuned chunk-clock across solo/party/raid; regime-detect + per-regime strategies behind the frozen API, with per-encounter rhythm profiles for the raid path (anchor [D-013-IJ]). Proven against real fights via the replay pipeline (see `docs/workorders/WO-069-IJ.md`). Status: In progress — Phase 1 regime DESIGN complete (fable+ultracode, unanimous "minimal seams"); design doc + D-014 (Proposed); awaiting acceptance before implementation.
