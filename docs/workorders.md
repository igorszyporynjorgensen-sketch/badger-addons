...
- **WO-078-IJ** — **investigate the learned opening baseline** (anchor [D-022-IJ](b), human idea): over the first W seconds measure the early kill rate and predict duration as `k / r` with a per-boss learned `k`. Promising on a first pass (median error at 3/5/8s: 61.1/54.3/43.2% vs the shipped estimator's ~70/66/60%, and vs naive linear's 1699/692/331%), and it targets the band where confidence is *inverted* — 69.0% error at 0.96 confidence in the first 2s. **The decisive question is whether it survives the rise throttle**, since WO-077's re-learned profiles were a -14.4 improvement alone and a significant regression (+1.42) once the throttle existed. **Answer: it works, scoped and capped.** The crossover is ~5s (the human called it mid-run: "would seem only useful IF useful for first few seconds"); the failure mode is division by a near-zero early rate (worst 14.2% to 10320%); and with a 5s handover plus a cap at 1.5x the history prior it IMPROVES (-1.05 mean, -1.17 median, CI [-1.74,-0.36], 64% of fights) — surviving the throttle, unlike WO-077's profiles. A control proves the cap alone changes 0% of fights, so the benefit is the baseline. Worth ~1 point whole-fight but **6-13 median points across the first 5 seconds**, where the estimator is worst and claims 0.96 confidence. Q5 (unknown boss / universal k) NOT answered and gates shipping (see `docs/workorders/WO-078-IJ.md`). Status: Done.
- **WO-077-IJ** — **the Molten Core spectrum pass** (anchor [D-013-IJ], builds on WO-076): 9 of 10 MC bosses have ZERO fixtures on disk, so there is no honest post-WO-076 MC baseline at all and the shipped profiles came from 25 train kills each, one phase, fast end. Measured floor first (median boss MAPE **34.9%**; **Sulfuron 105.3% / +26.0s**, reading ~double). Part A fixes three instrument defects that would corrupt a duration-spanning corpus (flat sample pooling weights a 320s kill 20x a 15s kill; Python/Lua sort mismatch makes "the train half" differ between tools; the evidence guard counts samples not kills). Part B builds a ~4500-fight corpus over 3 phase groups x 3 trimmed-quantile tiers with a free metadata manifest and train/val/test as directories. Part C re-learns and tests H1 (the "universal slow open" may be a fixed ~3s warm-up artifact, r=+0.732 with fight length) and H2 (the execute peak sits exactly at Warrior Execute's 20% and MC is melee-heavy — so it may belong to the players, not the bosses) (see `docs/workorders/WO-077-IJ.md`). Status: **Done (Rev B)** — corpus built (1,061 fixtures, audit-clean); held-out `test/` verdict: re-learned profiles are a **significant regression** (+1.42, CI [+0.44,+2.40]) once the rise throttle exists, so **no profile ships**. The win is D-022-IJ's throttle (−45.03, CI [−52.75,−37.31]), which needed no corpus. Originally: **Proposed (Rev B)** — a 12-agent adversarial review + a live HTTP 429 falsified four claims: the graders omit `priorRate` while the driver always passes it (so the floor table measures a player who has never killed the boss); the API budget is ~800 calls/hour so Rev A was a ~12-hour job not "under an hour"; WCL rankings are guild-deduplicated so "bottom of logged raids" means bad guilds' BEST nights and contains no wipes; and the decision is measurably stable from n=10, so ~60/boss buys the same answer as 450/boss.
- **WO-076-IJ** — **make the lab model the client (the sticky show gate)** (anchor: the PR #97 verification, [D-021-IJ]): `driver.lua:68` checks confidence only while `not wasShown`, so once the bar latches every later `confCap` — including a hard `0.0` — is unreachable, yet the graders gate per-tick and `learn-regime.py` derives caps per-bin independently. Caps were learned and graded under a rule the client does not implement (Viscidus bit-identical to no regime; Skeram/Sulfuron ~nullified; **Onyxia's `[11] = 0.0` air-phase cap dead — its worst bin, 45.9% rel / 59.0s abs over 200 fixtures**). Instrument-only: latch the gate in the three graders (+ extract the shared grader), stop the learner claiming unreachable bins hide, pass `damageable` honestly, fix `regimes_spec`'s kills-only case (D-019(b)), then present the re-learned-vs-shipped caps diff — **shipping no caps change** (see `docs/workorders/WO-076-IJ.md`). Status: **Done** — PR #100 merged (`bf6ed75`), gate green. Measured: **16 of 105 caps can never fire**, and grading honestly moves the corpus MAPE **55.6% → 70.2%** over 788 kills — the estimator was never as good as the lab said. No caps change shipped; candidates deliberately not regenerated (the committed ones predate the current corpus). Numbers + method: `docs/reference/wo076-lab-vs-client.md`. Open: the air phase needs a non-confidence mechanism (→ D-022-IJ).
- **WO-075-IJ** — **implement the regime structural-tail layer → ship 0.9.48** (anchor [D-014-IJ] design + [D-018-IJ] timing): the injected `opts.regime` table + nil-guarded health-anchored estimator seams (freeze/immune/reset off `h`, **cap confidence so the bar goes quiet**) that a rhythm profile provably can't do. Three human-merged PRs (freeze+hideBar+lab · confCap+suppressFlush · resetOnRise), each gated by sim byte-identity + corpus regression. Onyxia's air phase was the motivating case. Full design: `docs/reference/estimator-regime-design.md` (see `docs/workorders/WO-075-IJ.md`). Status: Done — PRs #95/#96/#97 merged; 14 profiles ship; freeze tier removed (D-020); 0.9.48 next.
- **WO-074-IJ** — **learn Onyxia's rhythm + ship 0.9.47** (anchor [D-013-IJ]): the one raid boss missing from the shipped library (no corpus before). Harvest Fresh Onyxia (id 151084), learn the profile (captures the ~50%-health air-phase dip), held-out-validated (MAPE 74.2%→52.7%), add to `rhythms.lua` (dual-key 1084+151084), cut 0.9.47 via the pipeline (see `docs/workorders/WO-074-IJ.md`). Status: Done — PR #94 merged; `badger-ttk/0.9.47` released.
- **WO-073-IJ** — **multi-output release via `nx release`** (anchor [D-017-IJ]): independent per-addon versioning off each `.toc` `## Version:` (custom `versionActions` under `tools/release/`, `currentVersionResolver: disk`), Conventional-Commits bump + per-project changelog, per-project tag `{projectName}/{version}` + zip + GitHub Release; build-repro (bash ≥4.3, pinned packager, monorepo staging) fixed; CurseForge stays manual + gated (D-015). Scaffold back-port gated on a real release cycle. Grounded in the installed Nx 23.1 API (see `docs/workorders/WO-073-IJ.md`). Status: Done — PR #92 merged.
- **WO-072-IJ** — **kill-history → D-012 record schema** (the 1.0.0 forward-compat MUST, anchor [D-015-IJ]): rewrite `history.lua` to per-kill records (encounter/creature split, size/comp/when, 50-cap) + driver record/prior + core migration (old running-mean data wiped). Closes the gap audit Tier-1 #3 (see `docs/workorders/WO-072-IJ.md`). Status: Done — PR #91 merged; main green.
...
- **WO-069-IJ** — **estimator regime-aware overhaul** (fable + ultracode): stop stretching the solo-tuned chunk-clock across solo/party/raid; regime-detect + per-regime strategies behind the frozen API, with per-encounter rhythm profiles for the raid path (anchor [D-013-IJ]). Proven against real fights via the replay pipeline (see `docs/workorders/WO-069-IJ.md`). Status: In progress — Phase 1 DESIGN complete + ACCEPTED (D-014); implementation is **WO-075**, pulled forward to ship as **0.9.48** (D-018), so 1.0.0 now includes the regime layer.
