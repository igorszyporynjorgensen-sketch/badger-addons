...
- **WO-069-IJ** — **estimator regime-aware overhaul** (fable + ultracode): stop stretching the solo-tuned chunk-clock across solo/party/raid; regime-detect + per-regime strategies behind the frozen API, with per-encounter rhythm profiles for the raid path (anchor [D-013-IJ]). Proven against real fights via the replay pipeline (see `docs/workorders/WO-069-IJ.md`). Status: In progress — WIRING SHIPPED (PR #85): nine MC profiles live behind opts.rhythm; awaiting in-game /reload. MC library complete (loops 1–10).
