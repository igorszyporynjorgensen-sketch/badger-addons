...
- **WO-044-IJ** — **ship the icons**: convert the PNGs to uncompressed TGA, ship `badger-ttk-icon.tga` + `badger-master.tga` in `Media/`, set `## IconTexture` (badger-ttk), add `tools/png-to-tga.py`, commit the source PNGs (see `docs/workorders/WO-044-IJ.md`). Status: Accepted.
