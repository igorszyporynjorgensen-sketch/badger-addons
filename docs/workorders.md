...
- **WO-070-IJ** — **corpus harvest + batch-grade** (the "learning how to learn" data engine): `tools/wcl-corpus.py` (WCL V1 rankings → many real kills across the performer spectrum → fixtures in a gitignored harvest zone) + `tools/estimator-batch.lua` (aggregate MAPE/bias/shown over the corpus). Feeds WO-069 (see `docs/workorders/WO-070-IJ.md`). Status: Accepted — building.
- **WO-069-IJ** — **estimator regime-aware overhaul** (fable + ultracode): stop stretching the solo-tuned chunk-clock across solo/party/raid; regime-detect + per-regime strategies behind the frozen API, with per-encounter rhythm profiles for the raid path (anchor [D-013-IJ]). Proven against real fights via the replay pipeline (see `docs/workorders/WO-069-IJ.md`). Status: Proposed — awaits human accept + the fable/ultracode session.
