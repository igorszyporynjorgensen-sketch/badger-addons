...
- **WO-065-IJ** — **config layout pass**: Behavior (drop intro+separator), Display row restructure (position/scale up top), Estimator (widen history toggles), Abilities (wider offset slider + header spacing); first WO under D-011 — no version bump (see `docs/workorders/WO-065-IJ.md`). Status: In progress — PR #69.
