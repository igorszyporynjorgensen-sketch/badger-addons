...
- **WO-069-IJ** — **estimator regime-aware overhaul** (fable + ultracode): stop stretching the solo-tuned chunk-clock across solo/party/raid; regime-detect + per-regime strategies behind the frozen API, with per-encounter rhythm profiles for the raid path (anchor [D-013-IJ]). Proven against real fights via the replay pipeline (see `docs/workorders/WO-069-IJ.md`). Status: In progress — loop 14 (PR #89): remaining raids swept; 41-profile library across five raids; 973/1025 held-out wins; /reload still deferred.
