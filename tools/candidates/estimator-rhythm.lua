...
-- WO-069 LOOP-1 CANDIDATE — a copy of src/engine/estimator.lua + the rhythm-profile injected
-- dependency (opts.rhythm). Iterated via `python3 tools/ttk-lab.py grade --est` against the frozen
-- corpus; the shipped estimator is untouched until the loop proves an improvement.
...
function Estimator:ttk()
    ...
    if self.rhythm and self.rhythm.bins and #self.rhythm.bins > 0 then
        -- Rhythm-aware remaining time: live rate calibrates the SCALE (this group's speed), the profile
        -- supplies the SHAPE (what the fight does next). m(h) = rate at health h relative to the fight
        -- average; implied average R = rate / m(hNow); remaining = S(h) / R with S(h) = integral of
        -- dh'/m over 0..h (piecewise-constant bins; bins[1] = the execute end). m == 1 everywhere
        -- reproduces lastH/rate exactly. The profile SUBSUMES the execute correction (no double-count).
