...
local function buildSkin(db)
    ...
    return {
        ...
        args = {
            ...
            colorHeader = { type = "header", name = "Colours", order = 20 },
            colorTarget = {
                type = "color",
                name = "Target (TTK)",
            ...
            colorUtility = {
                type = "color",
                name = "Utility (waiting)",
                desc = "A utility bar before its fire moment — the cooldown isn't due yet.",
                order = 22,
                hasAlpha = true,
            ...
            colorReady = {
                type = "color",
                name = "Utility (fire)",
                desc = "A utility bar at its fire moment — fire the cooldown now.",
                order = 23,
                hasAlpha = true,
            ...
            colorUsed = {
                type = "color",
                name = "Utility (fired)",
                desc = "A utility bar after it's been fired — its buff is draining.",
                order = 24,
                hasAlpha = true,
            ...
            colorFont = {
                type = "color",
                name = "Font",
                desc = "Colour of the text on every bar.",
                order = 25,
                hasAlpha = true,
                get = colorGetter(db, "colorFont"),
                set = colorSetter(db, "colorFont"),
            },
        },
    }
end
...
local function buildDisplay(db)
    return {
        ...
        args = {
            ...
            -- Rows (WO-065, renders by order): Lock+Reset / anchor+growth / H+V offset+Scale /
            -- width+spacing+maxBars / TTK+Util height / Frame strata / the three opacities.
            anchorPoint = {
                type = "select",
                name = "Screen anchor",
            ...
            ttkBarHeight = {
                type = "range",
                name = "TTK bar height",
                desc = "Height of the main time-to-kill bar, in pixels.",
                order = 6,
                width = "relative",
                relWidth = 0.49,
                min = 8,
                max = 60,
                step = 1,
                get = getter(db, "ttkBarHeight"),
                set = setterR(db, "ttkBarHeight"),
            },
            utilityBarHeight = {
                type = "range",
                name = "Utility bar height",
                desc = "Height of each utility (cooldown-timing) bar, in pixels.",
                order = 6.3,
                width = "relative",
                relWidth = 0.49,
                min = 8,
                max = 60,
                step = 1,
                get = getter(db, "utilityBarHeight"),
                set = setterR(db, "utilityBarHeight"),
            },
            barSpacing = {
                type = "range",
                name = "Bar spacing",
            ...
            opacity = {
                type = "range",
                name = "Opacity",
                desc = "Overall transparency of the whole display.",
                order = 8,
                width = "relative",
                relWidth = 0.33,
                min = 0,
            ...
            barFgOpacity = {
                type = "range",
                name = "Fill opacity",
                desc = "Opacity of the bar fill (the coloured part). 100% = fully solid.",
                order = 8.3,
                width = "relative",
                relWidth = 0.33,
                min = 0,
                ...
                step = 0.05,
                isPercent = true,
                get = getter(db, "barFgOpacity"),
                set = setterR(db, "barFgOpacity"),
            },
            barBgOpacity = {
                type = "range",
                name = "Background opacity",
                desc = "Opacity of the empty background track a bar reveals as it drains.",
                order = 8.6,
                width = "relative",
                relWidth = 0.33,
                min = 0,
            ...
            ttkTextX = {
                ...
                max = 60,
                step = 1,
                get = getter(db, "ttkTextX"),
                set = setterR(db, "ttkTextX"),
            },
            ttkTextY = {
                type = "range",
                name = "TTK text Y",
                ...
                max = 30,
                step = 1,
                get = getter(db, "ttkTextY"),
                set = setterR(db, "ttkTextY"),
            },
            utilTextX = {
                type = "range",
                name = "Utility text X",
                ...
                max = 60,
                step = 1,
                get = getter(db, "utilTextX"),
                set = setterR(db, "utilTextX"),
            },
            utilTextY = {
                type = "range",
                name = "Utility text Y",
                ...
                max = 30,
                step = 1,
                get = getter(db, "utilTextY"),
                set = setterR(db, "utilTextY"),
            },
            strata = {
                type = "select",
                name = "Frame strata",
            ...
            resetPosition = {
                ...
                order = 2.3,
                width = "relative",
                relWidth = 0.49, -- on the Screen anchor · Lock line (WO-054)
                func = function()
                    if ns.Display then
...
local function buildAbilities(db)
    ...
    for i = 1, #ns.AbilityTable do
        ...
        -- Gap so the toggle + description read as a "local header" above the offset/colour row (WO-065).
        warrior["sp" .. e.id] = {
            type = "description",
            name = " ",
            fontSize = "medium",
        ...
        warrior["o" .. e.id] = {
            ...
            order = base + 3,
            width = 1.5, -- ~3x a "half" so the slider isn't cramped (WO-065)
            min = -30,
            max = 60,
            step = 1,
